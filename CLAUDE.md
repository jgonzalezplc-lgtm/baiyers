...
- **Supplier Capability Intelligence, Fase 4 (local, sin commit/deploy todavía)**: matriz editable proveedor–ítem en `/listas/[id]/proveedores-confianza`, ranking por capacidad privada + preferencia + score, selección persistida dentro del JSON de la lista, cobertura accesible, warnings y explicaciones. No envía correos ni registra evidencia hasta confirmar la RFQ en Fase 5. No requiere migración. Probada en lectura contra una lista real de 10 ítems (sin capacidades existentes: devolvió correctamente 10 sin cobertura).
- **Supplier Capability Intelligence, Fase 5 (local, sin commit/deploy ni migración todavía)**: migración 026 crea `rfq_batches`/`rfq_batch_items`; preparación idempotente desde la matriz, revisión editable en `/listas/[id]/rfq`, un correo Gmail por proveedor con varios ítems, una conversación asociada a múltiples resultados y protección `delivery_uncertain` para no duplicar envíos ambiguos. El agente Gmail resuelve el batch a todos sus resultados y marca cada ítem respondido. Requiere aplicar 026 antes de probar en producción.
...
3. Aplicar y confirmar la migración 026; verificar visualmente Fases 4–5 con proveedores categorizados y enviar una RFQ agrupada de prueba desde una cuenta Gmail conectada.
