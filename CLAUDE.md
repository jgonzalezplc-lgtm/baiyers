...
- **Supplier Capability Intelligence, Fase 6 (local, sin commit/deploy todavía)**: `/listas/[id]/busqueda-complementaria` separa ítems sin cobertura de los ya cubiertos, prioriza los primeros y exige acción explícita para buscar alternativas de los segundos. El buscador existente acepta `busqueda_expandida`, consulta todas las fuentes, crea una sesión `expanded` enlazada a la última dirigida y registra feedback idempotente `missing_suppliers`. No requiere migración.
