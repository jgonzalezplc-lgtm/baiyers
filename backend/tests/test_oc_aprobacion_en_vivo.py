...
PAYLOAD = {
    ...
    "proveedor_email": "joaquin.gonzalez.pl@usach.cl", "cantidad": 1.0, "unidad": "und",
...
def test_aprobar_despues_de_preparar_ya_no_exige_regenerar(monkeypatch):
    ...
    assert creada["request"].unidad == "und"


def test_el_borrador_con_decimal_y_unidad_llega_intacto_a_la_oc(monkeypatch):
    monkeypatch.setattr(svc, "get_active_draft", lambda *_a, **_k: {
        "draft_type": "purchase_order", "payload": {**PAYLOAD, "cantidad": 2.75, "unidad": "L"},
    })
    monkeypatch.setattr(svc, "commit_draft", lambda *_a, **_k: None)
    monkeypatch.setattr("app.services.workflow_execution.obtener_workflow_activo", lambda *_a, **_k: None)
    creada = {}
    ...
    monkeypatch.setattr("app.routers.oc.crear_oc", fake_crear_oc)
    asyncio.run(svc.create_purchase_order(MagicMock(), actor(), draft_id="draft-1", notes=None, confirmed=True))
    assert creada["request"].cantidad == 2.75
    assert creada["request"].unidad == "L"
