...
export default function ProveedoresConfianzaPage() {
  ...
  return <>
    ...
    <div style={{ display: "flex", justifyContent: "flex-end", gap: 9, marginTop: 18 }}><BtnSecondary onClick={() => router.push(`/listas/${id}`)}>Cancelar</BtnSecondary><BtnSecondary disabled={guardando} onClick={() => void guardar()}>{guardando ? "Guardando…" : "Guardar matriz"}</BtnSecondary><BtnPrimary disabled={preparando || seleccion.size === 0} onClick={() => void prepararCorreos()}>{preparando ? "Preparando…" : "Preparar correos"}</BtnPrimary></div>
  </>;
}
