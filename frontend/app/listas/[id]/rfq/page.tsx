...
export default function RevisarRFQPage() {
  ...
  return <>
    ...
    {batches.length === 0 ? <Card><EmptyState icon={Mail} title="No hay correos preparados" description="Vuelve a la matriz, selecciona al menos un proveedor y prepara los borradores." /></Card> : <>
      <Card style={{ marginBottom: 16 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}><div><strong style={{ color: "var(--n-900)" }}>{batches.length} correos agrupados</strong><div style={{ color: "var(--n-500)", fontSize: 12.5, marginTop: 3 }}>{enviados} enviados · {batches.length - enviados} pendientes</div></div><div style={{ display: "flex", gap: 8, alignItems: "center" }}><Badge status={enviados === batches.length ? "aprobada" : "cotizando"}>{enviados === batches.length ? "Completado" : "En revisión"}</Badge><BtnSecondary onClick={() => router.push(`/listas/${id}/busqueda-complementaria`)}>Búsqueda complementaria</BtnSecondary></div></div></Card>
